 ...
 */
final class Version20200727142104 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__run AS SELECT id, cluster_id, program_id, start, label, description, date_end, status, uuid FROM run');
        ...
        $this->addSql('INSERT INTO run (id, cluster_id, program_id, start, label, description, date_end, status, uuid) SELECT id, cluster_id, program_id, start, label, description, date_end, status, uuid FROM __temp__run');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__run AS SELECT id, cluster_id, program_id, start, label, description, date_end, status, uuid FROM run');
        $this->addSql('DROP TABLE run');
        $this->addSql('CREATE TABLE run (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, cluster_id INTEGER DEFAULT NULL, program_id INTEGER DEFAULT NULL, start DATETIME DEFAULT NULL, label VARCHAR(255) DEFAULT NULL, description CLOB DEFAULT NULL, date_end DATETIME DEFAULT NULL, status VARCHAR(255) DEFAULT NULL, uuid VARCHAR(255) DEFAULT NULL)');
        $this->addSql('INSERT INTO run (id, cluster_id, program_id, start, label, description, date_end, status, uuid) SELECT id, cluster_id, program_id, start, label, description, date_end, status, uuid FROM __temp__run');
        $this->addSql('DROP TABLE __temp__run');
        $this->addSql('CREATE INDEX IDX_5076A4C0C36A3328 ON run (cluster_id)');
        $this->addSql('CREATE INDEX IDX_5076A4C03EB8070A ON run (program_id)');
